...
final class MapStamp {
    ...
    ///
    /// 한 변 720px로 잘라 담는다 (StickerMaker.outputSide). 원본을 그대로 담으면 한 장에
    /// 수 MB가 되어 스탬프 몇 십 개만으로 아이클라우드 동기화가 무거워진다. 720은 화면
    /// 가득 펼쳐 봐도(StickerViewer) 흐려지지 않는 가장 작은 크기에서 잡은 것이다.
    ///
