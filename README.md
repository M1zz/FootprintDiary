...
- **오늘의 발자취**: 지도 위 요약 판을 누르면 오늘 들른 곳이 시각 차례대로 섭니다.
  자리 사이에는 얼마를 걸어서 왔는지가 적힙니다. 걸음이 짚어 낸 자리 가운데 이미
  스탬프가 있는 곳은 "다녀오신 게 맞나요?"라고 물어, 답하면 그 자리의 방문 횟수가
  하나 늘어납니다. 아직 스탬프가 없는 자리는 거기서 바로 찍을 수 있습니다.
- **개척 나침반**: 지금 선 자리를 한가운데 두고 둘레 1.5km를 여덟 방위로 갈라,
  방위마다 얼마나 촘촘히 훑었는지(채움)와 얼마나 멀리 나가 봤는지(뻗음)를 잽니다.
  여덟 뻗음을 이은 도형이 걸어서 넓힌 땅의 생김새이고, 바늘은 그 도형이 가장 얕게
  팬 쪽을 가리킵니다. 판은 기기가 보는 쪽을 위로 두고 돕니다.
...
| `ExplorerCompass.swift` | 개척 나침반의 셈 — 여덟 방위의 채움과 뻗음 |
| `ExplorerCompassScreen.swift` | 개척 나침반 화면 (둥근 판·바늘·방위 표) |
| `DayTimeline.swift` | 오늘 들른 곳을 짚어 내는 셈 (머무름 + 걸음) |
| `DayTimelineScreen.swift` | 오늘의 발자취 화면 (자리 차례 + 다녀왔는지 묻기) |
